        ...
        <div className="header-inner">
          ...
          <nav className="desktop-nav" aria-label={isIndonesian ? "Navigasi utama" : "Main navigation"}>
            {navItems.map(([label, href]) => (
              <Link key={href} href={href} className={location === href ? "nav-link is-active" : "nav-link"}>{label}</Link>
            ))}
          </nav>
          <div className="header-actions">
            <SearchDialog />
            <button className="language-toggle" type="button" onClick={toggleLanguage} aria-label={copy.switchLanguage}>
              <Languages size={15} strokeWidth={1.8} />
              <span>{language === "id" ? "EN" : "ID"}</span>
            </button>
            <button className="menu-toggle" type="button" onClick={() => setMenuOpen((open) => !open)} aria-label={menuOpen ? copy.closeMenu : copy.openMenu} aria-expanded={menuOpen}>
